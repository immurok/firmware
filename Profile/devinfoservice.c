...
static uint8_t       devInfoSerialNumber[13] = "000000000000"; // 6-byte MAC as hex
...
static uint8_t devInfoPnpId[DEVINFO_PNP_ID_LEN] = {
    ...
    LO_UINT16(0xFFFF), HI_UINT16(0xFFFF), // Vendor ID (unassigned)
    LO_UINT16(0x0001), HI_UINT16(0x0001), // Product ID (IK-1)
    LO_UINT16(0x0001), HI_UINT16(0x0001)  // Product version
};
...
bStatus_t DevInfo_SetParameter(uint8_t param, uint8_t len, void *value)
{
    ...
    switch(param)
    {
        ...
        case DEVINFO_SERIAL_NUMBER:
            ...
            {
                tmos_memcpy(devInfoSerialNumber, value, len);
                devInfoSerialNumber[len] = '\0';
            }
            break;
