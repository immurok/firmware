...
/* Optional watchdog kick callback — called during long ECC computations */
static void (*g_uECC_watchdog_cb)(void) = 0;
void uECC_set_watchdog_cb(void (*cb)(void)) { g_uECC_watchdog_cb = cb; }
#define uECC_KICK_WATCHDOG() do { if (g_uECC_watchdog_cb) g_uECC_watchdog_cb(); } while(0)
...
uECC_VLI_API void uECC_vli_modInv(uECC_word_t *result,
    ...
    while ((cmpResult = uECC_vli_cmp_unsafe(a, b, num_words)) != 0) {
        if ((++_wdg_ctr & 63) == 0) { uECC_KICK_WATCHDOG(); }
        if (EVEN(a)) {
